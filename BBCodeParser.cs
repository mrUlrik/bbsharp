...
public partial class BBCodeDocument
{
    ...
    /// <param name="BBCode">A string of BBCode text</param>
    /// <param name="ThrowOnError">Whether to throw an exception on parse error. If false, the error is ignored</param>
    /// <param name="SingularTags">
    ///     A list of tags which should be considered singular by the parser. Singular tags are self
    ///     closing and may not have children.
    /// </param>
    /// <returns>The DOM representation of the text</returns>
    public static BBCodeDocument Load(string BBCode, bool ThrowOnError, IEnumerable<string> SingularTags)
    {
        var document = new BBCodeDocument();
        var nodestack = new Stack<BBCodeNode>();
        ...
        for (var i = 0; i < BBCode.Length; i++)
            ...
            else
            {
                ...
                if (i == BBCode.Length)
                {
                    ...
                    AddPlainText(document, nodestack, "[" + TagName);
                    continue;
                }


                var IsClosing = BBCode[i] == '/';
                if (IsClosing)
                    i++;
                ...
                else
                {
                    ...
                    AddPlainText(document, nodestack, "[" + TagName); // prepend the [ char which started the tag
                }
            }
        ...
        if (nodestack.Count > 0 && ThrowOnError)
            throw new BBCodeParseException("Reached end of document with " + (nodestack.Count - 1) + " unclosed tags",
                BBCode.Length);

        return document;
    }

    private static void AddPlainText(BBCodeDocument doc, Stack<BBCodeNode> stack, string text)
    {
